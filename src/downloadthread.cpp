...
bool DownloadThread::_openAndPrepareDevice()
{
    ...
    if (authopenresult == _file.authOpenCancelled) {
        ...
        msg += "<br>"+tr("Please verify if 'BeagleBoard Imager' is allowed access to 'removable volumes' in privacy settings (under 'files and folders' or alternatively give it 'full disk access').");
...
void DownloadThread::_onWriteError()
{
    ...
    if (_file.errorCode() == ERROR_ACCESS_DENIED)
    {
        ...
        {
            msg += "<br>"+tr("Controlled Folder Access seems to be enabled. Please add both bb-imager.exe and fat32format.exe to the list of allowed apps and try again.");
        }
...
void DownloadThread::setImageCustomization(const QByteArray &config, const QByteArray &cmdline, const QByteArray &firstrun, const QByteArray &cloudinit, const QByteArray &cloudInitNetwork, const QByteArray &initFormat, const QByteArray &sysconf)
{
    _config = config;
    _cmdline = cmdline;
    _firstrun = firstrun;
